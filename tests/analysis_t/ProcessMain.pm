...
sub new {
  my $class = shift;
	print "[YOO]sub new is called with $class\n";
...
# YOO START
  print "[YOO] new! self->type = $self->{_type}, path[0] = $paths, config = $config, yoo = $yoo\n";
# YOO END

  bless $self, $class;
	bless \$yoo, $class; # didn't work
#$self->run($self, $paths, $config, "$yoo");
	$self->run();
...
sub run {
  my ($self, $paths, $config, $yoo) = @_;
  print "[YOO] run! self->type = $self->{_type}, path = $self->{_paths}, config = $self->{_config}, yoo = $self->{_yoo}\n";
  print "[YOO] run2! yoo = $yoo"; # didn't work
...
sub outputIntermediaries {
  ...
  print "[YOO] outputIntermediaries! self->type = $self->{_type}, path = $self->{_paths}, config = $self->{_config}, yoo = $self->{_yoo}\n";
...
sub generateIntermediaries {
  ...
  foreach $path (keys %paths) {
    $value = $paths{$path};
		print "\n[YOO] in generateIntermediaries, \$value = $value\n";
      ...
      my $data_path = "Log4Analysis/"; #$self->{_config}->{"DataPath"};
      $data_path .= $path;
      $ps = `ls $data_path`;
      #print $ps . "\n";
      @files = split(/\n/, $ps);
		print "[YOO] generateIntermediaries, \@files = @files\n";
...
if (1) {
      ...
      my %loads2 = ();
      foreach $file (@files) {
		print "[YOO] generateIntermediaries, \$file = $file\n";
		...
		print "[YOO] before file split = $file\n";
        @a = split(/\^/, $file);
        $n = $a[-1]; # [YOO-C] changed
		print "[YOO] after file split = $a[0] + $a[1] + $a[-1]\n";
        if ($url) {
          if (($value ne "two" or $n eq "1")) { # When cold happens
            $loads1{$url}{$rp->getLoad()} = $file;
          }
          if ($n eq "2"  and $rp->getLoad() > $loads2{$url}) { # When Hot happens
      ...
      foreach $url (keys %loads1) {
		print "[YOO] calculate the corresponding file = $url\n";
        my %temp = %{$loads1{$url}};
...
if (0) { # hot
      ...
      #do real analysis on 
      #cold
		print "[YOO] COLD access is called!!!!!!!\n";
