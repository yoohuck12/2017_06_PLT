...
#_info = $_[0] = RawParser=HASH(0x1ef7050) and _load = $_[1] = 71113.124392
  print "[YOO] In Dependency, _info = \$_[0] = $_[0] and _load = \$_[1] = $_[1]\n";
  my $self = {
    _info => $_[0], #RawParser
    _load => $_[1], #DOMLoad
    _config => $config,
...
sub process {
  ...
  print "pageUrl: " . $url , "\n"; #pageUrl: searchengineland.com_4-areas-need-consider-performing-adwords-audits-274669
  print "pageStart: " . $pageStart , "\n"; #pageStart: 71100.951454

  @comps = @{$info->getComps()}; # array of Computation. e.g. execScript, recalcStyle
  @objects = @{$info->getObjects()}; # ObjectHash array
  %objecthashes = %{$info->getObjecthashes()}; # almost same w/ objects but ObjectHash's code is used to gather same objects which have same code

  @hols = @{$info->getHols()}; # start w/ HOL
  @mcaus = @{$info->getMatchedCSSAndUrls()};
  @preloads = @{$info->getPreloads()}; # Preloads Image, JS (pre-downloads it?)
  @resourceinfos = @{$info->getResourceinfos()}; # ?????

  ####################################
  # add obj_id to resources # Resources are paresd!!
  ...
  @parses = @{$self->{_parses}};
  @comps_post_n = @{$self->{_comps_post_n}}; #  no recalc is needed
  @comps_post_l = @{$self->{_comps_post_l}}; # css file should be recalculate the style during computation
...
sub addId2Resources {
  ...
  foreach $resource (@resources) {
    ...
    $resource{"blocking"} = $resource{"requestTime"} - $resource{"sentTime"}; # How long it is blocked
    $resource{"proxy"} = 0;
    $resource{"dns"} = 0;
    $resource{"conn"} = 0;
    $resource{"ssl"} = 0;
...
sub addComps2Parses {
  ...
  $info = $self->{_info};
  @comps = @{$info->getComps()};
  @parses = @{$self->{_parses}}; # Objects and ObjectHashes

  my @comps_post_n; # urlRecalcStyle is not exist
  my @comps_post_l; # urlRecalcStyle is exist --> Recalculation with css files
  ...
  foreach $comp (@comps) {
    ...
    foreach $parse (@parses) {
      %parse = %{decode_json($parse)};
	  # if both url in Object/ObjectHash(Resource) and url in Computation is same
      if ($parse{"url"} eq $comp{"docUrl"}) {  
        # During parsing
		# if code in Object/ObjectHash is different from code in Computation
        if ($parse{"last_code"} ne $comp{"code"}) {
#print "[YOO] last_code is ne to comp{code} \n";
            if ($parse{"during_n"}) {
...
#print "[YOO] \$parse{during_n} = $parse{"during_n"}+ "\n";
        ...
        # Post parsing
		# If code is same, just push urlRecalcStyle info comps_post_n or comps_post_n
        } else {
		  # if urlRecalcStyle is not exist --> type: most execScript, layout ...
          if ($comp{"urlRecalcStyle"} eq "(null)" or $comp{"urlRecalcStyle"} eq "") {
            push(@comps_post_n, $comp);
		  # if urlRecalcStyle is exist
          } else { # type: fireEvent, some execScript
...
print "receivedTime: " . ($r->{receivedTime}) . "\n";
    ...
    $obj->{download}->{len} = $r->{len};
    $obj->{download}->{receivedTime} = $r->{receivedTime};
    $obj->{download}->{dns} = $r->{dns};
    $obj->{download}->{sslEnd} = $r->{sslEnd};
    $obj->{download}->{sslStart} = $r->{sslStart};
    $obj->{download}->{dnsStart} = $r->{dnsStart};
    $obj->{download}->{dnsEnd} = $r->{dnsEnd};
    $obj->{download}->{sendEnd} = $r->{sendEnd};
    $obj->{download}->{sendStart} = $r->{sendStart};
    $obj->{download}->{receiveHeadersEnd} = $r->{receiveHeadersEnd};
    $obj->{download}->{receiveFirst} = $r->{receiveFirst};
    $obj->{download}->{receiveLast} = $r->{receiveLast};
    $obj->{download}->{connectStart} = $r->{connectStart};
    $obj->{download}->{connectEnd} = $r->{connectEnd};
    $obj->{comps} = [];
    push($graph->{objs}, $obj);
