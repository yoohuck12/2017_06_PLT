...
sub readFromFile {
  ...
  my @temp = split(/\//, $self->{_filename}); # split with /
  my $num_temp = @temp; # number of splited words
  $temp1 = $temp[$num_temp - 1]; #last one --> 
  @temp = split(/\+/, $temp1); # plit with +
  $num_temp = @temp;
  $self->{_domain} = $temp[0]; 
  ...
  my $str = '';
  while (<FH>) {
    $str .= $_; # insert strings of data file to str
  }
  close FH;
  $self->{_content} = $str;
}
...
sub parse {
  ...
  $self->{_isContentAvailable} = 1;
	# find the last domainname^log^1 (1 means cold) 
  @a = split(/\^/, $self->{_filename}); 
  my $filename = $a[0];
  print "[YOO] parse! \$filename = $filename\n";
  ...
  foreach my $line (@lines) {
	...
	#print "sentTime " . $resource{"sentTime"} . "\n";
      ...
      if (!$pageUrl) {
        ...
        $pageUrlTemp = $a[1];
        $pageUrlTemp =~ s/\//\_/g;
		print "[YOO] rawParser. \$filename = $filename, \$pageUrlTemp = $pageUrlTemp\n";
  ...
  # set to variable
  $self->{_resources} = \@resources; # chunks from ReceivedChunk ==> Network
  $self->{_objecthashes} = \%objecthashes; # meta, link, script, div ... 
  $self->{_objects} = \@objects;
  $self->{_comps} = \@comps;
  $self->{_hols} = \@hols;
  $self->{_matchedCSSAndUrls} = \@matchedCSSAndUrls; #DK
  $self->{_preloads} = \@preloads; # Don't know, DK
  $self->{_resourceinfos} = \@resourceinfos;

  $self->{_pageStart} = $start; #First Resource is called with \$filename url
  $self->{_pageEnd} = $end; # When DOMLoad Function is called
  if (!$self->{_dep}) {
    open FF, ">>ttt";
  ...
  $self->{_pageParseStart} = $pageParseStart; # When first Parsing starts -> objectHash with url domain
  $self->{_pageParseEnd} = $pageParseEnd;# When last Parsing ends -> objectHash with url domain
}
