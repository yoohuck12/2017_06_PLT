...
imp_type = 'minification'
new_json_dict = {'comp': {'osize': 0.00001794, 'otime': 0.81311926},
                 'download': {'osize': -0.0002752, 'otime':  0.92540063}}

"""new_json_dict = {'comp': {'osize': 0.00001, 'otime': 0.000311926},
                 'download': {'osize': 0.0002752, 'otime':  0.000092540063}}"""
...
def dataArrives(site_name):
    javascript_type_list = ['1', '2', '3', 'application:x-javascript', 'application:javascript', 'application:ecmascript',
                            'text:javascript', 'text:ecmascript', 'application:json', 'javascript:text']
    css_type_list = ['4', 'text/css', 'css:text']
    initial_time = -1
    ...
    for i in range(len(d['objs'])):
        ...
        if d['objs'][i]['download']:
            if d['objs'][i]['download']['receiveLast'] < 0:
                ...
                try:
                    ...
                    ###
                    #  Put the new learned comp_time here
                    ###
                    #obj_download['len'] = d['objs'][i]['download']['receiveLast']  # - d.objs[i].download.receiveFirst
                    if d['objs'][i]['download']['type'] in javascript_type_list or \
                       d['objs'][i]['download']['type'] in css_type_list or \
                       d['objs'][i]['url'].endswith('.js') or  \
                       d['objs'][i]['url'].endswith('.css'):
                        obj_download['len'] = d['objs'][i]['download']['len'] * new_json_dict['comp']['osize'] + \
                                          d['objs'][i]['download']['receiveLast'] * new_json_dict['comp']['otime']
                    else:
                        obj_download['len'] = d['objs'][i]['download']['receiveLast']  # - d.objs[i].download.receiveFirst
        ...
        for j in range(len(d['objs'][i]['comps'])):  # Goes through all of the comps
            ...
            ###
                ...
                #new_json_dict = {'comp': {'osize': 0.0002, 'otime': 0.8}}

            ###
            elif (d['objs'][i]['comps'][j]['type'] in javascript_type_list) or \
                    (d['objs'][i]['comps'][j]['type'] in css_type_list) or \
                    (d['objs'][i]['url'].endswith('.js')) or \
                    (d['objs'][i]['url'].endswith('.css')):
                obj_comps['len'] = d['objs'][i]['download']['len'] * new_json_dict['comp']['osize'] + \
                                   d['objs'][i]['comps'][j]['time'] * new_json_dict['comp']['otime']
            else:
                obj_comps['len'] = d['objs'][i]['comps'][j]['time']
    ...
    for index, values in enumerate(order):
        print(values['id'], 'start', values['start'],'oStart',  values['oStart'],'end',  values['end'],'info',  values['info'], 'url', values['url'].replace('http://original.testbed.localhost/', '') ,'len',  values['len'])
        print('prev', values['prev'])
